...
async def create_job(
    ...
    if job_type in JobTypes:
        ...
        if job_type == JobType.DEFAULT:
        ...
        elif job_type == JobType.EZSPEC_UNIDOCK:
            log.info(f"------------------ STARTING EZSPEC-UNIDOCK JOB ------------------  job[{job_type}]: " + job_id)
            job_config = json.loads(job_info.replace('\\"', '"'))
            ...
            # Upload job_config.json to MinIO (PDB files already uploaded via /upload endpoint)
            if service.ensure_bucket_exists(job_type):
                service.upload_file(
                    job_type,
                    f"/{job_id}/in/job_config.json",
                    json.dumps(job_config).encode('utf-8')
                )
