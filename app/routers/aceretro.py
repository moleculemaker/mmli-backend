...
# /aceretro/scoreFromSmile
# /aceretro/buildGraphFromAsync
# /aceretro/getChemoenzyPathAsync

@router.post("/aceretro/scoreFromSmile", tags=['ACERetro'])
async def score_from_smi(requestBody: AnalyzeRequestBody, background_tasks: BackgroundTasks, service: MinIOService = Depends(), db: AsyncSession = Depends(get_session), email_service: EmailService = Depends()):
    # Analyze only one document for NSF demo
    if requestBody.smile != "" and requestBody.jobId != "":
        # run score_from_smi
    # if len(requestBody.fileList) > 0 and requestBody.jobId != "":
        ...
        if not db_job:
            db_job = Job(
                ...
                job_info=separator.join(requestBody.smile),
        ...
        filename = f"{uuid.uuid4()}{requestBody.smile}"
        aceretroService = ACERetroService(db=db)
        objectPath = f"{requestBody.jobId}/in/{filename}"
        background_tasks.add_task(aceretroService.scoreFromSmile, 'aceretro', filename, objectPath, requestBody.jobId, service, email_service)
