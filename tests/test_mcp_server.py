...
def _call_as(client, client_ip, arguments, name="submit_novostoic_optstoic"):
    """Invoke a tool while presenting a particular client address."""
    response = client.post("/mcp", headers={**MCP_HEADERS, "X-Forwarded-For": client_ip},
...
class TestEndpointPath:
    """The endpoint must answer at the URL clients are configured with.

    A Starlette mount at "/mcp" matches only "/mcp/...", so bare "/mcp" falls through to
    redirect_slashes and answers 307. MCP clients are configured with the endpoint URL
    itself, so that costs every request an extra round trip and breaks any client that
    does not follow redirects on POST.
    """

    @pytest.mark.parametrize("path", ["/mcp", "/mcp/"])
    def test_the_endpoint_answers_without_redirecting(self, client, path):
...
class TestRateLimiting:
    ...
    def test_each_client_gets_its_own_budget(self, client):
        """Previously the key was a constant, so all callers shared one allowance.

        One busy agent could then exhaust the budget for every other client of the
        server, which is indistinguishable from the service being broken.
        """
        for _ in range(mcp_app.SUBMIT_LIMIT):
            _call_as(client, "10.0.0.1", OPTSTOIC)

        exhausted = _call_as(client, "10.0.0.1", OPTSTOIC)
        other_client = _call_as(client, "10.0.0.2", OPTSTOIC)

        assert exhausted["result"]["isError"] is True
        assert other_client["result"].get("isError") is not True
